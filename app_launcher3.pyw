...
# pyinstaller --add-data 'app_library.py;.' --add-data 'app_launcher.ico;.' --add-data 'Read Me.txt;.' --exclude-module app_library app_launcher3.pyw --contents-directory .
        ...
        def update_apps_file():
            with open("app_library.py", "w") as file:
                ...
                for app_name, app_data in apps.items():
                    file.write(f"\n    '{app_name}':{{'path':r'{app_data['path']}','icon':r'{app_data['icon']}'}},")
                file.write(f"\n}}")
...
#### Add Game Tab ####
    ...
    def ui_add_game():
        ...
        type_game = [("App files", "*.exe"),("All files", "*.*")]
        ...
        def input_game():
            if len(ui_add_game.lokasi_entry.get()) == 0 or len(ui_add_game.gambar_entry.get()) == 0:
                ...
                with open("app_library.py", "r+") as file:
                    ...
                    lines.insert(-1, ("    ' " + judul + "': {'path':r'" + lokasi +
                                        "','icon':r'" + gambar + "'}," + "\n"))
                    file.seek(0)
        ...
        ui_add_game.lokasi_label.grid(row=2, column=0, columnspan=1)

        ui_add_game.lokasi_entry = Entry(master=window, bg="white", width=30)
        ui_add_game.lokasi_entry.grid(row=2, column=1, columnspan=4)
        ...
        ui_add_game.lokasi_button.grid(row=2, column=5)

        ui_add_game.gambar_label = Label(master=window, text="Icon", anchor="w", compound='left')
        ui_add_game.gambar_label.grid(row=3, column=0, columnspan=1)

        ui_add_game.gambar_entry = Entry(master=window, bg="white", width=30)
        ui_add_game.gambar_entry.grid(row=3, column=1, columnspan=4)
...
#### Edit Game Tab ####
    ...
    def ui_edit_game():
        ...
        def edit_game(a,b,c):
            def input_game(a):
                if len(ui_edit_game.lokasi_entry.get()) == 0 or len(ui_edit_game.gambar_entry.get()) == 0:
                    ...
                    with open("app_library.py", "w") as file:
                        file.write(f"apps = {{" + "")
                        for app_name, app_data in apps.items():
                            file.write(f"\n    '{app_name}':{{'path':'{app_data['path']}','icon':'{app_data['icon']}'}},")
                        file.write(f"\n}}")
                    ...
                    with open("app_library.py", "r+") as file:
                        ...
                        window.destroy()
            type_icon = [("Image files", ("*.jpg", "*.jpeg", "*.png", "*.ico")),("All files", "*.*")]
            type_game = [("Game files", "*.exe"),("All files", "*.*")]
            def browse_game(a,b):
            ...
            ui_edit_game.lokasi_label.grid(row=2, column=0)

            ui_edit_game.lokasi_entry = Entry(master=window, bg="white", width=30)
            ui_edit_game.lokasi_entry.grid(row=2, column=1, columnspan=4)
            ...
            ui_edit_game.lokasi_button.grid(row=2, column=5)

            ui_edit_game.gambar_label = Label(master=window, text="Icon", anchor="w", compound='left')
            ui_edit_game.gambar_label.grid(row=3, column=0)

            ui_edit_game.gambar_entry = Entry(master=window, bg="white", width=30)
            ui_edit_game.gambar_entry.grid(row=3, column=1, columnspan=4)
        ...
        for app_name, app_data in app_items:
            ...
            button = Button(master=window, text=f" Edit       {app_name}", image=icon, compound='left', anchor="w",
                            width=300, command=lambda name= app_name, path = app_data['path'], image=app_data["icon"]: edit_game(name,path,image))
            button.image = icon
            button.grid(column=0, columnspan=5, padx=10, pady=5)
...
#### Buttons Remove Section ####
    ...
    button_edit = Button(master=window, width=14, text="Edit Game", command=lambda: [ui_edit_game(), remove_edit()])
...
for app_name, app_data in app_items_sorted:
    ...
    button = Button(root, text=app_name, image=icon, compound='left', anchor="w",
                       width=lebarlauncher, command=lambda app_path=app_data['path']: launch_app(app_path))
    button.image = icon 
...
button = Button(root, text='Edit App', image=icon, compound='left', anchor="w",
                   width=lebarlauncher, command=lambda : game_editor(0))
button.image = icon 
button.pack(padx=10, pady=5)
root.mainloop()
